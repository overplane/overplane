...
- Subcommands: git-style via the registry; no Cobra/Viper/Urfave. New commands register in `commandRegistry`, implement a command interface, provide `color.HelpSpec`, and map failures to the exit-code table.
...
- Quality: do not loosen gates, lower coverage, or skip linters. Fix root causes. Run `make ci` and `make ci-rehearse` before merge.
